...
public class CustomCalendarSu extends View {

    public int pxToDp(Context context, float px) {
        float density = context.getResources().getDisplayMetrics().density;
        Log.d(TAG, "pxToDp: " + density);
        Log.d(TAG, "pxToDp: " + (int) (px / density + 0.5f));
    ...
    public CustomCalendarSu(Context context) {
        this(context, null);
    }

    public CustomCalendarSu(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomCalendarSu(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int startX = (getHeight() - (cellSize*7))/2;
        Log.d(TAG, "onDraw: " + getHeight() + getWidth());
        int startY = (getHeight() - (cellSize*5))/2;

        // 绘制标题（年-月）
        mPaint.setTextSize(cellSize * 0.43f);
        Paint tmpPaint = new Paint(mPaint);
        tmpPaint.setColor(Color.RED);
        float TitleTextWidth = mPaint.measureText("月");
        canvas.drawText(String.format(Locale.getDefault(), "%d月", month + 1), startX+(cellSize - TitleTextWidth) / 2, (float) startY-cellSize*0.7f, tmpPaint);
        ...
        for (int dayOfMonth = 1; dayOfMonth <= daysInMonth; dayOfMonth++) {
            ...
            if (dayOfMonth == day) {
                ...
                tmpPaint.setColor(Color.RED);
                canvas.drawCircle(x + cellSize / 2,y + textHeight/2,cellSize/2.2f,tmpPaint);
